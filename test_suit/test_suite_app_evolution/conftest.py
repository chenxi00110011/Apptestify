...
def should_restart_and_update():
	"""
	检查是否在过去一小时内已经重启过设备。如果没有，则重启设备并更新日志文件。
	"""
	if not os.path.exists(LOG_FILE_PATH):
		...
		with open(LOG_FILE_PATH, 'r+') as f:
			last_restart_time_str = f.read().strip()
			last_restart_time = datetime.strptime(last_restart_time_str, '%Y-%m-%d %H:%M:%S')
			if datetime.now() - last_restart_time > REBOOT_INTERVAL:
				# 更新为当前时间
				f.seek(0)
				f.write(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
				f.truncate()
