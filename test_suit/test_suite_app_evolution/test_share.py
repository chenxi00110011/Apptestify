...
"""
==== 23 passed, 86 deselected, 102 warnings, 4 rerun in 8524.47s (2:22:04) ====
"""
...
parameter = [
    ...
    (account_config.get('睿博士正式服账号', '泰国手机账户_1'),
     ...
     account_config.get('睿博士正式服账号', '泰国区号')
     ),
    # 国际邮箱账户转让设备
    (account_config.get('睿博士正式服账号', '国际邮箱账户_1'),
     account_config.get('睿博士正式服账号', '国际邮箱密码_1'),
    ...
    (account_config.get('睿博士正式服账号', '美服手机账户_1'),
     ...
     account_config.get('睿博士正式服账号', '美国区号')
     ),
    # 美服邮箱账户转让设备
    (account_config.get('睿博士正式服账号', '美服邮箱账户_1'),
     account_config.get('睿博士正式服账号', '美服邮箱密码_1'),
    ...
    (account_config.get('睿博士正式服账号', '欧服邮箱账户_1'),
     ...
     account_config.get('睿博士正式服账号', '英国区号')
     )
]
...
@pytest.mark.repeat(1)
@pytest.mark.case_name("手机账户和邮箱账户输入账户分享设备")
@pytest.mark.flaky(reruns=3, reruns_delay=2)
@pytest.mark.share
@pytest.mark.parametrize("account, pwd, did, name, share_account, share_pwd, areaCode", parameter)
def test_account_sharing(account, pwd, did, name, share_account, share_pwd, areaCode):
    # 设备复位
    reset(did)
    time.sleep(15)
    ...
    app.run_parameters['rectangle'] = 1.0
    app.go_to_page('首页', account, pwd)
    app.go_to_page('分享给账号', name, '分享管理', share_account)
...
@pytest.mark.share
@pytest.mark.parametrize("account, pwd, did, name, share_account, share_pwd, areaCode", parameter)
def test_qr_sharing(account, pwd, did, name, share_account, share_pwd, areaCode):
    # 设备复位
    reset(did)
    time.sleep(15)
...
@pytest.mark.share
@pytest.mark.parametrize("account, pwd, did, name, share_account, share_pwd, areaCode", parameter)
def test_WeChat_sharing(account, pwd, did, name, share_account, share_pwd, areaCode):
    # 设备复位
    reset(did)
    time.sleep(15)
