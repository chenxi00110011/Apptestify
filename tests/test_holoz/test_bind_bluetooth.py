...
@pytest.mark.device_binding_buletooth
@pytest.mark.repeat(200)
def test_device_binding_buletooth(setup_holoz: Uiautomator2SophisticatedExecutor):
    app = setup_holoz
    ...
    while not (app.exists_element(selector="text", value="设备添加成功") or
        ...
        if count >= 60:
            break
    end_time = time.time()
    app.title['wakeup_time'] = '{:.1f}'.format(end_time - start_time)
